...
UBYTE tile_at_boat(UBYTE tx, UBYTE ty) {
  UBYTE _tile = tile_at(tx, ty);
  if (_tile & 0x80) {
    if (!VM_GLOBAL(VAR_HAS_BOAT)) {
    ...
    if ((topdown_grid == 16 && ON_16PX_GRID(PLAYER.pos)) ||
        ...
        while (tile_start_y != tile_end_y) {
            ...
            while (tile_start_x != tile_end_x) {
                UBYTE tile = tile_at_boat(tile_start_x, tile_start_y);
                tile_collision |= tile;
                tile_start_x++;
            }
        ...
        if (INPUT_RECENT_LEFT) {
            ...
            while (tile_start != tile_end) {
                ...
                UBYTE tile = tile_at_boat(tile_x - 1, tile_start);
            ...
            while (tile_start != tile_end) {
                ...
                UBYTE tile = tile_at_boat(tile_start, tile_y - 1);
